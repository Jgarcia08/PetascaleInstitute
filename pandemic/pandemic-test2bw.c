...
/*    if (our_rank == 0) 
        printf("p%3s: %18s, %18s, %18s, %18s\n", "", "init", "computation", "communication", "rankSize");

    printf("p%3d: %18.15f, %18.15f, %18.15f, %18.15f\n", our_rank, initTime - beginTime, compTime, totalCommTime, rsTime);
    //fflush(stdout);

    if (MPI_Barrier(MPI_COMM_WORLD) != MPI_SUCCESS) {
        fprintf(stderr, "error in barrier of every process\n"); 
        exit(-1);
    }
*/    if (our_rank % 32 == 16) {
        buf[0] = beginTime;
    ...
    //fflush(stdout);
    if (MPI_Barrier(MPI_COMM_WORLD) != MPI_SUCCESS) {
        fprintf(stderr, "error in 16's barrier\n");
        exit(-1);
    }
    ...
    printf("rank %d finalized in %.15f s\n", our_rank, finTime);
    //fclose(out);
